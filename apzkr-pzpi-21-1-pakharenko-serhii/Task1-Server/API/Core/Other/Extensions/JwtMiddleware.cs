...
public class JwtMiddleware(RequestDelegate next)
{
    ...
    private void AttachUserToContext(HttpContext context, IJwtService jwtService, string token)
    {
        try
        {
            ...
            var isGuard = Enum.TryParse<Role>(jwtToken.Claims.FirstOrDefault(x => x.Type == ClaimTypes.Role && x.Value == nameof(Role.GuardRole))?.Value ?? "", out var role);
            
            context.Items["IsGuard"] = isGuard;
